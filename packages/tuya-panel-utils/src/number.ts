 ...
 * @returns {String}
 */
const toFixedString: (num: number, count: number) => string = (num: number, count: number) => {
  let s = `${Math.abs(num)}`;
 ...
 * @returns {String}
 */
const toFilledString: (num: number, count: number) => string = (num: number, count: number) => {
  let s = `${Math.abs(num)}`;
 ...
 */
const numToHexString: (num: number, padding?: number) => string = (num, padding = 2) => {
 ...
 */
const numToByteNumbers: (num: number, bytes?: number) => number[] = (num, bytes = 2) => {
  const hex = Number(num).toString(16);
...
const calcPosition: (
  ...
  newMin: number,
  newMax: number
) => number = (value, min, max, newMin, newMax) => {

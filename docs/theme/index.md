...
> 你可以在本地主题配置文件中(src/config/theme.js)覆盖所有主题配置（所有主题变量可见下面主题速查表），当然也可以自行在项目中拓展自己业务项目的主题配置；
...
const slider = {
  /**
   * 如果以下 key 值存在，则优先级相比 slider[theme.type] 来说是更高的。
   */
  // width: null,
  // trackRadius: 2,
  // trackHeight: 4,
  ...
  light: {
     ...
     * 注意此处，
     * 变量也可为动态的值，
     * 比如当前值将会跟随全局 brand 品牌色走，也可以像下面这样:
     * minimumTrackTintColor: props => props.disabled ? rgba(0,0,0,0.3): rgba(0, 0, 0, 1)
     */
 ...
 */
declare function getTheme(props: IPropsWithTheme, key: string, defaultValue: string | number): string | number;
```

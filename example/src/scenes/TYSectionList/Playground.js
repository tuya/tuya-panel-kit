...
export default class TYSectionListPlaygroundScene extends Component {
  state = {
    arrow: false,
    disabled: false,
    showIcon: false,
  ...
  get datas() {
    ...
    return [1, 2, 3].map(v => ({
      key: v,
      styles: this.state.isDefaultTheme ? {} : {
  ...
  renderPlayground = () => {
    return (
      <View>
        ...
        <ControlBoolean
          title="Toggle theme"
          value={this.state.isDefaultTheme}
          onValueChange={this._handleBoolChange('isDefaultTheme')}
        />
      </View>
    );
  }

...
describe('Tab Component', () => {
  it('basic render', () => {
    const component = renderer.create(
      <Tab defaultActiveKey='1'>
        <Tab.TabPane key='1' tab='1'><Text>{`The No.1 Tab`}</Text></Tab.TabPane>
      </Tab>
    );
  ...
  it('should props update', () => {
    const component = renderer.create(
      <Tab activeKey='2'>
        <Tab.TabPane key='1' tab='1'><Text>{`The No.1 Tab`}</Text></Tab.TabPane>
        <Tab.TabPane key='2' tab='2'><Text>{`The No.2 Tab`}</Text></Tab.TabPane>
      </Tab>
    );
  ...
  it('should scroll content', () => {
    ...
    const component = renderer.create(
      <Tab defaultActiveKey='1' onChange={jest.fn()} ref={ref => { mockRef = ref; }}>
        <Tab.TabPane key='1' tab='1'><Text>{`The No.1 Tab`}</Text></Tab.TabPane>
        <Tab.TabPane key='2' tab='2'><Text>{`The No.2 Tab`}</Text></Tab.TabPane>
      </Tab>
    );
    ...
    expect(instance.tabContent.state.scrollX._value).toBe(-300);
  });
  ...
  it('should tab bar layout', () => {
    ...
    expect(instance.tabBar.state.tabContainerWidth).toBe(750);
  });
});

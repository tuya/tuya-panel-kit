...
export default class ColorPicker extends Component {
  ...
  static correctionHSB(hsb) {
    ...
    const _s =
      s < minSaturation ? minSaturation : s > maxSaturation ? maxSaturation : s;
    const _b =
      b < minBrightness ? minBrightness : b > maxBrightness ? maxBrightness : b;
    return [h, _s, _b];
  }
    ...
    this._panResponder = PanResponder.create({
      onStartShouldSetPanResponder: e => {
        const {
          xRelativeOrigin,
          yRelativeOrigin,
        } = this.getStartOriginCoordByTouchEvent(e);
        const distance = this.getLengthTouchPointToCenterPoint(
          xRelativeOrigin,
          yRelativeOrigin
        );
  ...
  eventHandle(xRelativeOrigin, yRelativeOrigin, fn) {
    ...
    } = this.fixXYRelativeOrigin(xRelativeOrigin, yRelativeOrigin);
    this.thumbXY = this.getThumbCoord(
      xFixedRelativeOrigin,
      yFixedRelativeOrigin
    );
  ...
  updateThumb(saturation) {
    ...
    const { x, y } = this.mapHsbToXYRelativeOrigin([
      hue,
      saturation,
      brightness,
    ]);
